...
# Extract the KAS RSA public key from the private key and save it to "$opt_output/kas-cert.pem".
# This replaces the certificate that would have been generated by the original command.
...
elif [ "$(uname -m)" = "arm64" ] && sysctl -n machdep.cpu.brand_string 2>/dev/null | grep -q "M4"; then
