...
3. Create an OpenTDF config file: `opentdf.yaml`
   ...
   2. The `opentdf-example-no-kas-ext-ers.yaml` file is simpler to run but less secure. This file configures the platform to startup without a KAS instances, without a built-in ERS instance, and without endpoint authentication.
