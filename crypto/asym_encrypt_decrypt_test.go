...
func TestAsymEncryptionAndDecryption(t *testing.T) {
	...
	// Test 2048 key
	testAsymEncryptAndDecrypt(`-----BEGIN PRIVATE KEY-----
...
func testAsymEncryptAndDecrypt(privateKeyInPem string, publicKeyInPem string, t *testing.T) {
	asymEncryptor, err := CreateAsymEncryption(publicKeyInPem)
	if err != nil {
	...
	asymDecryptor, err := CreateAsymDecryption(privateKeyInPem)
	if err != nil {
	...
	if string(decryptedText) != plainText {
		t.Fatal("Asym encrypt/decrypt failed")
	}
}
