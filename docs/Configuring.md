...
| `kas_uri_from_kao`       | Use the key access object's KAS URI for rewrap key lookups instead of the indexer's default registration. | `true` |
...
By default, rewrap looks up keys using the KAS URI in the key access object (KAO). If that URI is absent or empty, it uses the key indexer's default registration, normally selected by `registered_kas_uri` (or the service's KAS URL when unset). A failed lookup does not retry another registration.

> [!IMPORTANT]
> The setting defaults to `true` because the KAO identifies the registration associated with the TDF's key, allowing a KAS to serve keys split across multiple registrations. This is the intended long-term behavior; `false` is a compatibility override for deployments that deliberately replace that registration. Making the override opt-in helps identify deployments that still need it before considering removal of the configuration flag.

Set `kas_uri_from_kao: false` to always use the indexer's default registration, for example when old KAS URLs are proxied to a replacement KAS whose keys are registered under its new URI. Disabling this setting emits an informational startup log only when key management is enabled. The public-key endpoint is unaffected, and in-process key indexes ignore the URI.
