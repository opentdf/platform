...
func ValidatePublicKeyPEM(pemBytes []byte) (*PublicKeyInfo, error) {
    // Walk all PEM blocks until we find a supported one
    data := pemBytes
    for {
        ...
        case "PUBLIC KEY":
            ...
            return classifyPublicKey(pub, SourcePublicKey)
        case "CERTIFICATE":
            cert, err := x509.ParseCertificate(block.Bytes)
            if err != nil {
                return nil, fmt.Errorf("%w: %v", ErrInvalidPublicKey, err)
            }
            return classifyPublicKey(cert.PublicKey, SourceCertificate)
        default:
            // try next block if present
            if len(rest) == 0 {
                return nil, ErrInvalidPEMBlock
            }
            data = rest
