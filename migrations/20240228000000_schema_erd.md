...
nodes: |
  ...
  currently supports the relation of SubjectMappings -> SubjectSets -> ConditionGroups (AND/OR) -> Conditions
  ...
  Attribute Values via SubjectMappings, but will enable flexibility to grammar and relation changes in the
  ...
  (utilizing our proto Messages/fields for SubjectSets, ConditionGroups, and Conditions) to 
  ...
  or more Conditions. The Conditions are where Subject context fields and values (i.e. a `username`
  ...
  NOTE: a PDP should consider all of the following joined by an AND relationship:
  ...
  NOTE: due to the many-to-many relation between SubjectMappings and SubjectConditionSets, a pivot table will be
