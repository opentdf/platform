...
func TestPlannerPlanMarksActionAlreadyMigratedWithoutMetadata(t *testing.T) {
	...
	assert.Equal(t, targetAction.GetId(), plan.Actions[0].Targets[0].ExistingID)
...
func TestPlannerPlanAllScopesBuildsAllPlanSections(t *testing.T) {
	...
	assert.Equal(t, []string{legacyAction.GetId()}, plan.SubjectMappings[0].Target.ActionSourceIDs)
	...
	assert.Equal(t, legacyAction.GetId(), plan.RegisteredResources[0].Target.Values[0].ActionBindings[0].SourceActionID)
...
func TestPlannerPlanInteractiveReviewerLeavesCurrentUnresolvedPlanShapeUntouched(t *testing.T) {
	...
	assert.Nil(t, plan.RegisteredResources[0].Target)
}
...
func TestPlannerPlanHuhInteractiveReviewerResolvesRegisteredResourceConflict(t *testing.T) {
	...
	require.NotNil(t, plan.RegisteredResources[0].Target)
	assert.Equal(t, TargetStatusCreate, plan.RegisteredResources[0].Target.Status)
	assert.True(t, sameNamespace(namespaceOne, plan.RegisteredResources[0].Target.Namespace))
	require.Len(t, plan.RegisteredResources[0].Target.Values, 1)
	require.Len(t, plan.RegisteredResources[0].Target.Values[0].ActionBindings, 1)
	assert.Equal(t, "action-legacy", plan.RegisteredResources[0].Target.Values[0].ActionBindings[0].SourceActionID)
	require.Len(t, plan.Actions, 1)
	require.Len(t, plan.Actions[0].Targets, 1)
	assert.Equal(t, TargetStatusCreate, plan.Actions[0].Targets[0].Status)
