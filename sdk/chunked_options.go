...
// There is deliberately no option to omit schemaVersion on its own.
// Omitting it is how a reader is told the TDF predates 4.3.0, and such a
// reader expects hex-then-base64 signatures -- which are written during
// WriteSegment, long before any Finalize option is seen. The two settings can
// only be chosen together, at construction, which is what WithChunkedTargetMode
// does.
