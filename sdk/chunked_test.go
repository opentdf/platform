...
// TestChunkedIntegrityAlgorithmRejectsUnsupported verifies both
// integrity-algorithm options reject a value outside {HS256, GMAC}
// rather than letting it reach calculateSignature, which treats any
// unrecognized value as GMAC.
func TestChunkedIntegrityAlgorithmRejectsUnsupported(t *testing.T) {
	ctx := context.Background()
	kasBundle := newChunkedFakeKAS(t)
	defer kasBundle.server.Close()

	const bogus IntegrityAlgorithm = 99

	for _, tc := range []struct {
		name string
		opt  ChunkedWriterOption
	}{
		{"root", WithChunkedIntegrityAlgorithm(bogus)},
		{"segment", WithChunkedSegmentIntegrityAlgorithm(bogus)},
	} {
		t.Run(tc.name, func(t *testing.T) {
			writer, err := NewChunkedWriter(ctx,
				WithChunkedDefaultKAS(kasBundle.simpleKey()),
