...
//
// xorSplitter holds no fields by design, not just by convenience:
// [sdk.KeySplitter] documents that Split may be called concurrently on the
// same instance, so nothing about one call may be cached on the struct for a
// later call to observe. A field added here later -- e.g. to memoize a
// splitter by default KAS -- would reintroduce exactly that hazard.
...
func (xorSplitter) Split(ctx context.Context, attrs []*policy.Value, dek []byte, defaultKAS *policy.SimpleKasKey) (*sdk.SplitResult, error) {
	...
	for url, key := range res.KASPublicKeys {
		out.KASPublicKeys[url] = sdk.KASPublicKey{
			// keysplit carries the algorithm as a bare string; sdk types
			// it, so name the conversion here rather than letting the
			// two drift apart silently. SplitResult.Validate rejects a
			// value ocrypto cannot parse.
