...
//   - An HS256 root signature and GMAC segment hashes
...
//   - The TDF spec version to target, via [WithTargetMode]
//
// Returns an error if:
...
//   - An integrity algorithm other than [RootHS256] or [SegmentGMAC] was asked for
//
// Example:
//
...
//
//	// Targeting a pre-4.3.0 reader
//	writer, err := NewWriter(ctx, WithTargetMode("4.2.2"))
func NewWriter(ctx context.Context, opts ...Option[*WriterConfig]) (*Writer, error) {
	...
	// [sdk.ChunkedWriter] emits an HS256 root and GMAC segments and has no
	// knob for either, so anything else the caller asked for is refused here
	// rather than silently ignored.
	if config.rootIntegrityAlg != RootHS256 {
		return nil, fmt.Errorf("%w: %s", sdk.ErrUnsupportedRootIntegrityAlgorithm, config.rootIntegrityAlg)
	}
	if config.segmentIntegrityAlg != SegmentGMAC {
...
//
// Returns a SegmentResult whose TDFData reads the encrypted segment bytes that
// should be stored/uploaded, and any error. Those bytes include ZIP structure
// elements. They must be concatenated in ascending index order to form the
// payload, whatever order they were produced in.
//
// Error conditions:
...
//
//	// Store/upload each segment's bytes (e.g., to S3), reading TDFData
//	uploadToS3(segment0.TDFData, "part-000")
//	uploadToS3(segment1.TDFData, "part-001")
