...
func TestSegmentWriter_SequentialOrder(t *testing.T) {
	...
	for i, data := range testSegments {
		...
		assert.Greater(t, len(segmentBytes), 0, "Segment %d should have bytes", i)
		...
		if i == 0 {
			...
			assert.Equal(t, len(data), len(segmentBytes), "Segment %d should be raw data", i)
		}
...
func TestSegmentWriter_OutOfOrder(t *testing.T) {
	...
	for _, index := range writeOrder {
		...
		assert.Greater(t, len(bytes), 0, "Segment %d should have bytes", index)
	...
	if err := os.WriteFile("/tmp/debug_out_of_order.zip", allBytes, 0644); err == nil {
...
func TestSegmentWriter_EmptySegments(t *testing.T) {
	...
	assert.Greater(t, len(finalBytes), 0, "Should have final bytes")
