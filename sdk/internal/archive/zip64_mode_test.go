...
func TestZip64Mode_Auto_Small_UsesZip32(t *testing.T) {
    ...
    if err != nil { t.Fatalf("zip open failed: %v", err) }
    if len(zr.File) != 2 { t.Fatalf("expected 2 files, got %d", len(zr.File)) }

    // Validate payload can be read and CRC matches implicitly.
    var payload *zip.File
    for _, f := range zr.File {
        if f.Name == TDFPayloadFileName { payload = f; break }
    }
    if payload == nil { t.Fatal("payload not found") }
    rc, err := payload.Open()
    if err != nil { t.Fatalf("open payload failed: %v", err) }
    _, err = io.ReadAll(rc)
    rc.Close()
    if err != nil { t.Fatalf("read payload failed: %v", err) }
}
...
func TestZip64Mode_Never_Overflow_Fails(t *testing.T) {
    ...
    if _, err := w.WriteSegment(ctx, 0, []byte("x")); err != nil { t.Fatal(err) }
