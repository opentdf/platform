...
// TestReaderZip64ExtraFieldOrder covers finding 2: APPNOTE 4.5.3 orders the
// ZIP64 values original size, compressed size, local header offset. Reading
// the compressed size first is invisible for STORED entries where the two
// match, so the fixture makes them differ.
...
// TestReaderZip64ExtraFieldNotFirst covers finding 4: the ZIP64 field is not
// required to head the extra-field area.
...
// TestReaderPerEntryZip64WithoutZip64EOCD covers the other half of finding 4:
// a ZIP64 extra field on an entry in an archive whose EOCD is not ZIP64.
...
// TestReaderCentralDirectoryFileComment covers the first half of finding 5:
// a comment on one entry must not desync the entries that follow it.
...
// TestReaderCentralDirectoryLengthOverflow covers the second half of finding
// 5: the three uint16 lengths must be widened before they are summed. Here
// they total 65646, which wraps to 110 at 16 bits.
...
// TestReaderZip64DetectedFromEntryCount covers finding 3: the entry count is
// its own ZIP64 trigger, and it is two bytes wide.
...
// TestWriterSwitchesToZip64AtInjectedThreshold covers finding 1. The
// production switch point is 2 GiB, which no unit test can reach without
// allocating a 2 GiB payload, so the threshold is lowered instead -- the same
// seam java-sdk uses.
...
// TestCentralDirectoryNarrowingGuard covers finding 6: a value that cannot
// fit the 32-bit field has to fail loudly instead of being truncated into a
// corrupt archive.
