...
func TestParseBaseUrl(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Standard KAS URL strips /kas suffix (service at platform root)",
			input:       "http://localhost:8080/kas",
			expected:    "http://localhost:8080",
			expectError: false,
		},
		{
			name:        "Legacy /v2/rewrap suffix stripped",
			input:       "https://kas.example.com/v2/rewrap",
			expected:    "https://kas.example.com",
			expectError: false,
		},
		{
			name:        "Combined /kas/v2/rewrap suffix stripped",
			input:       "https://kas.example.com/kas/v2/rewrap",
			expected:    "https://kas.example.com",
			expectError: false,
		},
		{
			name:        "Base-path KAS URL keeps base path, strips /kas suffix",
			input:       "https://kas.example.com/base/kas",
			expected:    "https://kas.example.com/base",
			expectError: false,
		},
		{
			name:        "Base-path KAS URL without known suffix is preserved",
			input:       "https://kas.example.com/base/path",
			expected:    "https://kas.example.com/base/path",
			expectError: false,
		},
		{
			name:        "Trailing slash is trimmed to avoid doubled path separator",
			input:       "https://kas.example.com/base/",
			expected:    "https://kas.example.com/base",
			expectError: false,
		},
		{
			name:        "IPv6 host without port keeps brackets, strips /kas suffix",
			input:       "https://[::1]/kas",
			expected:    "https://[::1]",
			expectError: false,
		},
		{
			name:        "IPv6 host with port keeps brackets and port",
			input:       "https://[::1]:8080/base",
			expected:    "https://[::1]:8080/base",
			expectError: false,
		},
		{
