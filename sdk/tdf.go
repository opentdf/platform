...
func generateWrapKeyWithXWing(publicKeyPEM string, symKey []byte) (string, error) {
	pubKeyRaw, err := ocrypto.XWingPubKeyFromPem([]byte(publicKeyPEM))
	if err != nil {
		return "", fmt.Errorf("generateWrapKeyWithXWing: failed to parse X-Wing public key: %w", err)
	}
	wrappedDER, err := ocrypto.XWingWrapDEK(pubKeyRaw, symKey)
	if err != nil {
