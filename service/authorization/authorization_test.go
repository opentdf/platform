...
func Test_GetDecisionsByTokenAllOf_Pass(t *testing.T) {
	logger, err := createTestLogger()
	require.NoError(t, err)
	...
	// set the request
	req := authorization.GetDecisionsByTokenRequest{DecisionRequests: []*authorization.TokenDecisionRequest{
		{
			Actions: []*policy.Action{},
			Tokens: []*authorization.Token{
				{
					Id:  "tok1",
					Jwt: tdf_entity_resolution_client_credentials_jwt,
				},
			},
			ResourceAttributes: []*authorization.ResourceAttribute{
				{AttributeValueFqns: []string{mockFqn1}},
	...
	resp, err := as.GetDecisionsByToken(ctxb, &req)
