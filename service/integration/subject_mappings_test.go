...
func (s *SubjectMappingsSuite) Test_ListSubjectMappings_NoPagination_Succeeds() {
	...
	for _, sm := range listed {
		if sm.GetId() == fixture1.ID {
			...
			s.Equal(sm.GetAttributeValue().GetFqn(), "https://example.com/attr/attr1/value/value1")
			found1 = true
		}
		if sm.GetId() == fixture2.ID {
			assertEqual(sm, fixture2)
			s.Equal(sm.GetAttributeValue().GetFqn(), "https://example.com/attr/attr1/value/value2")
			found2 = true
		}
