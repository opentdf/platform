...
type CasbinV2Config struct {
	...
	// GormDB is the GORM database connection for SQL-backed policy storage.
	// When provided, policies are stored in PostgreSQL instead of CSV.
	// If nil and Adapter is nil, falls back to CSV adapter.
	GormDB *gorm.DB

	// Schema is the database schema for the casbin_rule table.
