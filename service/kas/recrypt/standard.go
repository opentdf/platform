...
func (s *Standard) Derive(k KeyIdentifier, ephemeralPublicKeyBytes []byte) (UnwrappedKey, error) {
	...
	// client public bits
	ephemeralECDHPublicKey, err := ocrypto.ECPubKeyFromPem(ephemeralPublicKeyBytes)
	if err != nil {
		return nil, fmt.Errorf("derive: ocrypto.ECPubKeyFromPem failed: %w", err)
	}

	// shared secret
	sessionKey, err := ecdhKey.ECDH(ephemeralECDHPublicKey)
	if err != nil {
		return nil, fmt.Errorf("derive: ecdhKey.ECDH failed: %w", err)
	}
...
func NewECKeyPair() (*ecdh.PrivateKey, error) {
