...
const createNonDataResourceGroup = `-- name: CreateNonDataResourceGroup :one
...
type CreateNonDataResourceGroupParams struct {
	Name     string `json:"name"`
	Metadata []byte `json:"metadata"`
}
...
//	VALUES ($1, $2)
//	RETURNING id
func (q *Queries) CreateNonDataResourceGroup(ctx context.Context, arg CreateNonDataResourceGroupParams) (string, error) {
	row := q.db.QueryRow(ctx, createNonDataResourceGroup, arg.Name, arg.Metadata)
	var id string
	err := row.Scan(&id)
	return id, err
}
...
const deleteNonDataResourceGroup = `-- name: DeleteNonDataResourceGroup :execrows
...
//
//	DELETE FROM non_data_resource_groups WHERE id = $1
func (q *Queries) DeleteNonDataResourceGroup(ctx context.Context, id string) (int64, error) {
	result, err := q.db.Exec(ctx, deleteNonDataResourceGroup, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const deleteNonDataResourceValue = `-- name: DeleteNonDataResourceValue :execrows
/*
    VALUES
*/
...
//
//	DELETE FROM non_data_resource_values WHERE id = $1
func (q *Queries) DeleteNonDataResourceValue(ctx context.Context, id string) (int64, error) {
	result, err := q.db.Exec(ctx, deleteNonDataResourceValue, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const getNonDataResourceGroup = `-- name: GetNonDataResourceGroup :one
SELECT
    id,
    name,
    JSON_STRIP_NULLS(
...
type GetNonDataResourceGroupRow struct {
	ID       string `json:"id"`
	Name     string `json:"name"`
	Metadata []byte `json:"metadata"`
}
...
//	WHERE
//	    ($1::uuid IS NULL OR id = $1::uuid)
func (q *Queries) GetNonDataResourceGroup(ctx context.Context, id pgtype.UUID) (GetNonDataResourceGroupRow, error) {
	row := q.db.QueryRow(ctx, getNonDataResourceGroup, id)
	var i GetNonDataResourceGroupRow
...
const listNonDataResourceGroups = `-- name: ListNonDataResourceGroups :many
WITH counted AS (
...
type ListNonDataResourceGroupsParams struct {
	Offset int32 `json:"offset_"`
	Limit  int32 `json:"limit_"`
}

type ListNonDataResourceGroupsRow struct {
	ID       string `json:"id"`
	Name     string `json:"name"`
	Metadata []byte `json:"metadata"`
	Total    int64  `json:"total"`
...
//	LIMIT $2
//	OFFSET $1
func (q *Queries) ListNonDataResourceGroups(ctx context.Context, arg ListNonDataResourceGroupsParams) ([]ListNonDataResourceGroupsRow, error) {
	rows, err := q.db.Query(ctx, listNonDataResourceGroups, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListNonDataResourceGroupsRow
	for rows.Next() {
		var i ListNonDataResourceGroupsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Metadata,
...
type listPublicKeyMappingsParams struct {
	Offset      int32       `json:"offset_"`
	Limit       int32       `json:"limit_"`
	KasID       interface{} `json:"kas_id"`
	KasName     interface{} `json:"kas_name"`
