...
func readEphemeralPublicKey(reader io.Reader, curve eccMode) ([]byte, error) {
...
func ReadNanoTDFHeader(reader io.Reader) (*nanoTdf, error) {
	...
	if err := binary.Read(reader, binary.BigEndian, &nanoTDF.EphemeralPublicKey); err != nil {
		return nil, errors.Join(ErrNanoTdfRead, err)
	}
	ephemeralPublicKeyBytes, err := readEphemeralPublicKey(reader, nanoTDF.binding.bindingBody)

	pubKey, err := pubKeyByteToKey(ephemeralPublicKeyBytes)

	nanoTDF.EphemeralPublicKey = pubKey
...
// byteToKey converts a public key from []byte format (either DER or PEM encoded)
// to an *ecdsa.PublicKey. It returns an error if the conversion fails.
// This code was generated by ChatGPT
func pubKeyByteToKey(pubKeyBytes []byte) (*ecdh.PublicKey, error) {
	// Try to decode the public key from PEM format first
	block, _ := pem.Decode(pubKeyBytes)
	if block != nil {
		// If PEM decoding is successful, use the DER bytes from the decoded block
