...
func GenerateTempKeys(outputPath string) {
	...
	cosignPassword := "changeme"

	keyBytes, err := cosign.GenerateKeyPair(func(_ bool) ([]byte, error) {
		return []byte(cosignPassword), nil
	})
	if err != nil {
		log.Fatalf("Failed to create cosign key pair: %v", err)
	}
	policyImportPath := path.Join(outputPath, "policyimportexport")
	err = os.MkdirAll(policyImportPath, 0o755)
	if err != nil {
		log.Fatalf("Failed to create policy import export directory: %v", err)
	}

	if err := os.WriteFile(path.Join(policyImportPath, "cosign.pass"), []byte(cosignPassword), 0o600); err != nil {
		log.Fatalf("Failed to write cosign.pass file: %v", err)
	}

	if err := os.WriteFile(path.Join(policyImportPath, "cosign.pub"), keyBytes.PublicBytes, 0o600); err != nil {
		log.Fatalf("Failed to write cosign.pub file: %v", err)
	}

	if err := os.WriteFile(path.Join(policyImportPath, "cosign.key"), keyBytes.PrivateBytes, 0o600); err != nil {
		log.Fatalf("Failed to write cosign.key file: %v", err)
	}
}
