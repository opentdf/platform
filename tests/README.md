...
- `tests`: contains cucumber test suites.
...
```shell
go test ./tests/platform_test.go -v --tags=cukes --godog.tags="@fast,@unit"
```
...
```shell
go test ./tests/platform_test.go -v --tags=cukes --godog.tags="@fast and not @slow"
```
...
```shell
go test ./tests/platform_test.go --tags=cukes -v --godog.random ./features
```

#### Run only specific test suite for against explicit features
```shell
go test ./tests/platform_test.go -v --tags=cukes --godog.random ./your-custom-features
```

#### Run all tests

```shell
go test ./tests -v --tags=cukes --godog.random --godog.format=pretty
```

With logging to console for both cukes and containers:
```
CUKES_LOG_HANDLER=console COMPOSE_LOG_HANDLER=console go test ./tests -v --tags=cukes --godog.random --godog.format=pretty
```

### Run scenarios filtered by tags
```shell
go test ./tests -v --tags=cukes --godog.random --godog.tags=example ./features
```
...
- Package path: github.com/opentdf/platform/tests
...
```shell
go test ./tests -v --tags=cukes --godog.random <feature path>
```
